...
export default async function Page({ params }: { params: PageParams }) {
  const result = await (
    ...
    graphql(`
      query YoutubeSourcePage($sourceId: String!) {
  ...
  if (!result.findYoutubeVideoSource) notFound();
  const { embedUrl, sourceId } = result.findYoutubeVideoSource;
    ...
    >
      <div className={clsx("flex items-center gap-x-4 px-2")}>
        <h2 className={clsx("block text-lg font-bold text-snow-darker")}>
          Youtubeの<span className={clsx("font-mono")}>{sourceId}</span>
          としての情報
        </h2>
      </div>
      <iframe
