...
export const isOk = <TError, TData>(res: Result<TError, TData>): res is Ok<Result<TError, TData>> =>
  res.status === "ok";
export type ReturnOk<TFn extends (...args: never) => unknown> = Ok<Awaited<ReturnType<TFn>>>;
...
export const isErr = <TError, TData>(res: Result<TError, TData>): res is Err<Result<TError, TData>> =>
  res.status === "error";
export type ReturnErr<TFn extends (...args: never) => unknown> = Err<Awaited<ReturnType<TFn>>>;
